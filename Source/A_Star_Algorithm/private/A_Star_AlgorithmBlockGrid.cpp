...
//다음 위치 예외 상황 검사-여기서 false로 리턴이라면 경로에 적합하지 않다는뜻
bool AA_Star_AlgorithmBlockGrid::CheckException(FVector2D p_CheckPosition)
{
	...
	//X_정해진 범위 이내 ( 0 미만거나 GridSize 이상)
...
void AA_Star_AlgorithmBlockGrid::SelectTargetBlock(class AA_Star_AlgorithmBlock * p_TargetingBlock)
{
	...
	if (Cur_StartBlock == nullptr || Cur_TargetBlock == nullptr)	return;  //블록 지정 확인
...
{
	if (GetArriveTarget(p_Parent->GetPoistion(), p_Target)) return p_Parent;	/*부모 노드 도착여부 확인*/
...
TArray<FVector2D> AA_Star_AlgorithmBlockGrid::GetPath_While(FVector2D p_Startposition, FVector2D p_TargetPosition)
{
	...
	while (OpenList.Num() > 0)
	{
		...
		//F값? 시작부터 현재까지 거리+ 남은 목표 까지의 거리 절댓값
		...
		OpenList.Remove(CurrentNode);//현재 노드 오픈리스트 지우기
		ClosedList.Push(CurrentNode);//닫힌 노드에 삽입
		...
		if (GetArriveTarget(CurrentNode->GetPoistion(), p_TargetPosition))
		{
			...
			{
				FinalPathList.Push(TargetCurrent_node->GetPoistion()); //최종경로에 목표지점부터 push
				TargetCurrent_node = TargetCurrent_node->GetParent(); //해당 노드의 부모노드를 가져옴( 다음 갈 길)
			}

			FinalPathList.Push(p_Startposition); //처음 시작지점 마지막에 넣어줌

			ReverseArray(); //배열 역으로 뒤집기 보통 List였다면 간단하게 했을텐데, TArray사용으로 인해 따로 만들어줌

			return FinalPathList; //함수 리턴
		}
...
void AA_Star_AlgorithmBlockGrid::MovingTimer()
{
	...
	else
	{
		...
		DrawPath(Cur_DrawPosition, Cur_PathCountNum); //경로 블록 그리기(위치,번호)
	}
	Cur_PathCountNum++;
}
